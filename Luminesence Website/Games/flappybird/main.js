/*
 *
 * @author lumi Rosenstrom
 * http://www.lumi.net
 * https://twitter.com/lumi
 * http://www.devstate.net
 * Sources for this can be found at:
 * https://github.com/lumi/lumiNetCommonJS
 */


(function (window){

    var lumi = window.lumi || {};
    window.lumi = window.lumi || lumi;
      
      lumi.MathUtil = {};
      
      //used for radiansToDegrees and degreesToRadians
      lumi.MathUtil.PI_180 = Math.PI/180;
      lumi.MathUtil.ONE80_PI = 180/Math.PI;
      
      //precalculations for values of 90, 270 and 360 in radians
      lumi.MathUtil.PI2 = Math.PI*2;
      lumi.MathUtil.HALF_PI = Math.PI/2;
  
  
      //return number between 1 and 0
      lumi.MathUtil.normalize = function(value, minimum, maximum){
          return (value - minimum) / (maximum - minimum);
      };
  
      //map normalized number to values
      lumi.MathUtil.interpolate = function(normValue, minimum, maximum){
          return minimum + (maximum - minimum) * normValue;
      };
  
      //map a value from one set to another
      lumi.MathUtil.map = function(value, min1, max1, min2, max2){
          return lumi.MathUtil.interpolate( lumi.MathUtil.normalize(value, min1, max1), min2, max2);
      };
  
      lumi.MathUtil.getRandomNumberInRange = function(min, max){
          return min + Math.random() * (max - min);
      };
      
      lumi.MathUtil.getRandomIntegerInRange = function(min, max){
          return Math.round(lumi.MathUtil.getRandomNumberInRange(min, max));
      };
  ...
  (function (window){
      ...
      lumi.Geom.Point = function (x,y){
          this.x = isNaN(x) ? 0 : x;
          this.y = isNaN(y) ? 0 : y;
      };
  
      lumi.Geom.Point.prototype.clone = function(){
          return new lumi.Geom.Point(this.x,this.y);
      };
  
      lumi.Geom.Point.prototype.update = function(x, y){
          this.x = isNaN(x) ? this.x : x;
          this.y = isNaN(y) ? this.y : y;
      };
  
      lumi.Geom.Point.prototype.equals = function(point){
          return this.x==point.x && this.y==point.y;
      };
  
      lumi.Geom.Point.prototype.toString = function(){
      ...
      lumi.Geom.Rectangle = function (x, y, width, height){
          this.update(x, y, width, height);
      };
      
      lumi.Geom.Rectangle.prototype.update = function(x, y, width, height){
          this.x = isNaN(x) ? 0 : x;
          this.y = isNaN(y) ? 0 : y;
      ...
      lumi.Geom.Rectangle.prototype.getRight = function(){
          return this.x + this.width;
      };
      
      lumi.Geom.Rectangle.prototype.getBottom = function(){
          return this.y + this.height;
      };
  
      lumi.Geom.Rectangle.prototype.getCenterX = function(){
          return this.x + this.width/2;
      };
  
      lumi.Geom.Rectangle.prototype.getCenterY = function(){
          return this.y + this.height/2;
      };
  
      lumi.Geom.Rectangle.prototype.containsPoint = function(x, y){
          return x >= this.x && y >= this.y && x <= this.getRight() && y <= this.getBottom();
      };
  
      
      lumi.Geom.Rectangle.prototype.clone = function(){
          return new lumi.Geom.Rectangle(this.x, this.y, this.width, this.height);
      };
      
      lumi.Geom.Rectangle.prototype.toString = function(){
  ...
  /**
   * Created by lumi on 27-1-14.
   * has a dependecy on lumi.Geom
   * has a dependecy on lumi.BitmapUtil
   */
  ...
  (function (window){
      ...
      lumi.CanvasTextUtil.getFontSizeForRect = function(string, fontProps, rect, canvas, fillStyle){
          if(!canvas){
      ...
      lumi.CanvasTextProperties = function(fontWeight, fontStyle, fontSize, fontFace){
      ...
      lumi.CanvasTextProperties.NORMAL = "normal";
      lumi.CanvasTextProperties.BOLD = "bold";
      lumi.CanvasTextProperties.BOLDER = "bolder";
      lumi.CanvasTextProperties.LIGHTER = "lighter";
  
      lumi.CanvasTextProperties.ITALIC = "italic";
      lumi.CanvasTextProperties.OBLIQUE = "oblique";
  
  
      lumi.CanvasTextProperties.prototype.setFontWeight = function(fontWeight){
          switch (fontWeight){
              case lumi.CanvasTextProperties.NORMAL:
              case lumi.CanvasTextProperties.BOLD:
              case lumi.CanvasTextProperties.BOLDER:
              case lumi.CanvasTextProperties.LIGHTER:
                  this.fontWeight = fontWeight;
                  break;
              default:
                  this.fontWeight = lumi.CanvasTextProperties.NORMAL;
          }
      };
  
      lumi.CanvasTextProperties.prototype.setFontStyle = function(fontStyle){
          switch (fontStyle){
              case lumi.CanvasTextProperties.NORMAL:
              case lumi.CanvasTextProperties.ITALIC:
              case lumi.CanvasTextProperties.OBLIQUE:
                  this.fontStyle = fontStyle;
                  break;
              default:
                  this.fontStyle = lumi.CanvasTextProperties.NORMAL;
          }
      };
  
      lumi.CanvasTextProperties.prototype.setFontSize = function(fontSize){
      ...
      lumi.CanvasTextProperties.prototype.clone = function(){
          return new lumi.CanvasTextProperties(this.fontWeight, this.fontStyle, this.fontSize, this.fontFace);
      };
  
      lumi.CanvasTextProperties.prototype.getFontString = function(){
  ...
  function commitResize(){
      ...
      htmlBounds = new lumi.Geom.Rectangle(0,0, getWidth(canvasContainer) , getHeight(canvasContainer));
      ...
      bounds = new lumi.Geom.Rectangle(0,0, canvas.width, canvas.height);
      context.clearRect(0,0,canvas.width, canvas.height);
  ...
  //========================
      ...
      var scrollSpeed = 3;
      var score;
      var fontProperties = new lumi.CanvasTextProperties(lumi.CanvasTextProperties.BOLD, null, 100);
      ...
      function createLogo(logoText, logoCanvas, logoCanvassBG){
          ...
          var textRect = new lumi.Geom.Rectangle(0, 0, logoCanvas.width * .8, logoCanvas.height);
          var logoFontProps = fontProperties.clone();
          logoFontProps.fontSize = lumi.CanvasTextUtil.getFontSizeForRect(logoText, fontProperties, textRect);
      ...
      var collisionPoint = new lumi.Geom.Point();
      var birdPoints = [];
      ...
      var characters;
      var birdFontProperties = new lumi.CanvasTextProperties(lumi.CanvasTextProperties.BOLD, null, 50);
      ...
      function createTubes(){
          ...
          for(var i = 0; i < totalTubes; i++){
              ...
              tubes[i].topRect = new lumi.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
              tubes[i].bottomRect = new lumi.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
              renderTube(tubes[i]);
          }
          currentTube = tubes[0];
      }
  ...
  function createCityGraphic(){
      ...
      for(iterator=0; iterator<canvas.width; iterator+=minCloudRadius){
          cgContext.beginPath();
          cgContext.arc( iterator , maxCloudRadius, lumi.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius), 0, lumi.MathUtil.PI2);
          cgContext.closePath();
          cgContext.fill();
      }
      ...
      for(iterator=0; iterator<canvas.width; iterator+=(houseWidth+8)){
          ...
          houseHeight = lumi.MathUtil.getRandomNumberInRange(cityGraphicHeight *.5 , cityGraphicHeight - maxCloudRadius *.8);
      ...
      for(iterator=0; iterator<canvas.width; iterator+=minTreeRadius){
          cgContext.beginPath();
          radius = lumi.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius)
          cgContext.arc( iterator , cityGraphicHeight, radius, 0, lumi.MathUtil.PI2);
          cgContext.closePath();
          cgContext.fill();
      ...
      var sand = "#dcd795";
      var groundGraphicRect = new lumi.Geom.Rectangle();
